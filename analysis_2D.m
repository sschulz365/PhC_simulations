...
%Enter the effective slab and bulk material indices
n_eff = 2.82;
n_bulk = 3.48;

%Enter the field fraction contained within the slab
field_fraction = 0.827;
...
fileToSave_rho_contributions = [filename '_rho_contributions.fig'];
fileToSave_rho_contributions_k = [filename '_rho_contributions_k.fig'];
...
lambda = (n_eff/n_bulk)*a./freq;

%incoherent addition of gamma and rho
rho = (rho_first + rho_second + rho_third)*field_fraction;
gamma = (gamma_first + gamma_second + gamma_third)*field_fraction;
...
figure(1)
plot(lambda(1:39), ng(1:39), '-x'), hold on
plot(lambda(1:39), abs(ng(1:39)), '-xr')
...
figure(3)
plot(abs(ng(1:39)), loss(1:39), '-x'), hold on
...
figure(4)
plot(abs(ng(1:39)), loss_time(1:39), '-x'), hold on
xlabel('modulus of Group index');
...
figure(5)
plot(lambda(1:39), loss(1:39), '-x'), hold on
xlabel('Wavelength in nm');
ylabel('loss in dB/ns');
title(filename);
...
figure(6)
plot(abs(ng(1:39)), rho(1:39), 'r'), hold on
plot(abs(ng(1:39)), rho_first(1:39), 'b');
plot(abs(ng(1:39)), rho_second(1:39), 'g');
plot(abs(ng(1:39)), rho_third(1:39), 'k');
xlabel('modulus of Group index');
ylabel('rho a.u.')
title(filename)
...
%Plotting (and saving) the group index curve
figure(8)
plot(k(1:39), ng(1:39), '-x'), hold on
plot(k(1:39), abs(ng(1:39)), '-xr')
xlabel('k in 2\pi/a');
ylabel('Group index');
legend('ng', '|ng|')
title(filename);
